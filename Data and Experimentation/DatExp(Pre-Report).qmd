...
# part - 1
X1 <- X[which(X<breakingPoint)]
Y1 <- Y[which(X<breakingPoint)]
n = 10
lr = 1e-4
for(i in 1:n){
...
der_inv_a <- function(X,Y,a,b,c){
  ret <- 0 
  for(i in 1:length(X)){
    sqrt_term <- sqrt(b^2 - 4*a*(c-Y[i]))
    if(X[i]+((b/(2*a)) - sqrt(b^2 - 4*a*(c-Y[i]))/(2*a)) >0){
...
X2 <- X[which(X>=breakingPoint)]
Y2 <- Y[which(X>=breakingPoint)]
...
plot(X,Y, xlab = "CPI", ylab = "SENSEX", main = "L1 Norm Optimisation", cex.main = 2)
lines(as.numeric(X),as.numeric(py),col = "red")
